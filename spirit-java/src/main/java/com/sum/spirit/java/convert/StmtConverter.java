	...
	public void convert(IClass clazz, Element element) {
		...
		if (element.isSync()) {// sync s {
			...
			if (!token.isType() && token.isVar() && token.isDerived())
				element.addToken(1, new Token(TokenEnum.TYPE, TypeBuilder.build(clazz, token.getTypeAtt())));
			...
			element.replace(0, element.size(), new Token(TokenEnum.CUSTOM_EXPRESS, text));

		} else if (element.isAssign()) {// var = list.get(0)
			Token token = element.getToken(0);
			if (token.isVar() && token.isDerived())
				element.addToken(0, new Token(TokenEnum.TYPE, TypeBuilder.build(clazz, token.getTypeAtt())));
			...
			if (TypeUtils.isStr(type)) {
				...
				element.replace(1, element.size() - 1, new Token(TokenEnum.CUSTOM_EXPRESS, text));
			}

		} else if (element.isPrint() || element.isDebug() || element.isError()) {
			if (element.isPrint()) {
				element.setToken(0, new Token(TokenEnum.CUSTOM_PREFIX, "logger.info("));
			} else if (element.isDebug()) {
				element.setToken(0, new Token(TokenEnum.CUSTOM_PREFIX, "logger.debug("));
			} else if (element.isError()) {
				element.setToken(0, new Token(TokenEnum.CUSTOM_PREFIX, "logger.error("));
			}
			element.addToken(new Token(TokenEnum.CUSTOM_SUFFIX, ");"));
