...
@Component
@Order(-40)
public class InvocationVisitAction extends AbstractAppElementAction {
            ...
            for (int index = 0; index < statement.size(); index++) {
                try {
                    ...
                    if (token.isTypeSmartBuilder()) {
                        ...
                        setTypeForSmartBuilder(token.getValue(), parameterTypes);
    ...
    public void setTypeForSmartBuilder(Statement statement, List<IType> parameterTypes) {
        if (!parameterTypes.isEmpty()) {
            ...
            for (int index = 0; index < statements.size(); index++) {
                ...
                if (subStatement.size() == 1) {
                    Token token = subStatement.firstToken();
                    if (token.isTypeSmartBuilder()) {
                        token.setAttr(Attribute.TYPE, parameterTypes.get(index));
                    }
                }
            }
        }
